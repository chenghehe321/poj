...
public class MainBackup {
	...
	public int search(Sticks stick) {
		...
		for (int i = 0; i < candidates.size(); i++) {
			...
			if (groups > num || average < lens[0]) {// 剪枝1：分组组数超过木棍数量剪去，每组总和小于最长木棍剪去
				continue;
			}
			if (groups == 1) {
				return average;
	...
	private boolean sum(Sticks sticks, int start, int sum, int left,
		...
		for (int i = start; i < num; i++) {
			...
			if (sticks.get(i) > left) {
				continue;
			}
			if (!isSubsumEnough(sticks, i, left)) {
				break;
			}
			left -= sticks.pick(i);
			...
			while (i < num - 1 && sticks.get(i) == sticks.get(i + 1)) {
				i++;
			}
		}
		// if (left < 0) {
		...
		// }
		// while (start < num
		// && (sticks.isPicked(start) || sticks.get(start) > left)) {//
		// 剪枝5：尽快跳转到能适合当前木棍的最长木棍
		// five++;
		// start++;
		// }
		// if (start == num)
		// return false;
		// if (!isSubsumEnough(sticks, start, left)) {//
		// 剪枝4：如果剩余的木棒全部加起来都不能拼成，直接返回
		// four++;
		// return false;
		// }
		// left -= sticks.pick(start);
		// // System.out.print("+" + sticks.get(start) + ":" + left + ",");
		// result = sum(sticks, start + 1, sum, left, false);
		// if (!result) {//
		// left += sticks.unpick(start);
		// // System.out.print("-" + sticks.get(start) + ":" + left + ",");
		// if (isGoOn) {// 剪枝3：如果第一个木棍没找到任何匹配，直接返回
		// three++;
		// return false;
		// } else {
		// if (sticks.get(start) == left) {//
		// 剪枝6：最后一根合适的木棍不合适退出时，一定是退出多个，一个是不可能的，所以直接返回
		// six++;
		// return false;
		// }
		// while ((start < num - 1 && sticks.get(start) == sticks//
		// 剪枝2：跳过重复不适合的木棍
		// .get(start + 1))) {
		// two++;
		// start++;
		...
		// if (start >= num - 1)
		// return false;
		// return sum(sticks, start + 1, sum, left, false);
		// }
		// }
		return result;
